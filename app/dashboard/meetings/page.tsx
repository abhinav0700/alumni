...
export default async function MeetingsPage() {
  ...
  // Fetch upcoming meetings from MongoDB
  let upcomingMeetings = []
  try {
      // Use a direct import from the API route
      const { GET } = await import('@/app/api/meetings/route');
      const response = await GET();
      
      if (response.ok) {
        upcomingMeetings = await response.json();
        console.log('Meetings fetched successfully:', upcomingMeetings.length);
      } else {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
    } catch (error) {
      console.error('Error fetching meetings:', error);
      // Fallback to direct API call if import approach fails
      try {
        const meetingsResponse = await fetch('/api/meetings');
        if (meetingsResponse.ok) {
          upcomingMeetings = await meetingsResponse.json();
          console.log('Meetings fetched successfully (fallback):', upcomingMeetings.length);
        }
      } catch (fallbackError) {
  ...
  // Filter upcoming meetings (meetings in the future)
  const now = new Date()
  const upcoming = upcomingMeetings.filter((meeting: any) => new Date(meeting.meeting_date) > now)
  const pastMeetings = upcomingMeetings.filter((meeting: any) => new Date(meeting.meeting_date) <= now).slice(0, 5)
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {upcoming && upcoming.length > 0 ? (
              upcoming.map((meeting) => {
                ...
                const isRegistered = registeredMeetingIds.has(meeting._id)
          ...
          <div>
            ...
            <div className="space-y-4">
              {pastMeetings.map((meeting) => {
                ...
                return (
                  <Card key={meeting._id} className="border-border/50 bg-muted/20">
