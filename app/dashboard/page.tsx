...
export default async function DashboardPage() {
  ...
  let totalJobs = 0
  try {
    // Use direct import from the API route - this method is working correctly
    ...
    if (response.ok) {
      ...
      console.log('Jobs fetched successfully:', totalJobs);
    } else {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const now = new Date();
      const upcoming = allMeetings.filter(meeting => new Date(meeting.meeting_date) >= now);
      upcomingMeetings = upcoming.slice(0, 3);
      totalMeetings = upcoming.length;
      console.log('Meetings fetched successfully:', totalMeetings);
    } else {
